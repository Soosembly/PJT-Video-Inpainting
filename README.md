...
**3. E2FGVI** Inpainting
  </br>Masking된 영상을 Input으로 하여, 복원해야 할 누락된 지점으로 인식한다.
  </br>이 과정에서 알고리즘은 주변의 픽셀 정보를 사용하여 누락된 부분의 색생과 텍스처 등을 추정하고 채운다.

**4. Gradio** 라이브러리를 사용하여 사용자 친화적인 인페인팅 그래픽 사용자 인터페이스(GUI)를 구성함.
